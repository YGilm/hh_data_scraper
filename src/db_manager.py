...
class DBCreate:
    ...
    @staticmethod
    def pretty_print(data: Union[tuple, list]):
        if isinstance(data, list):
            for company, vacancies in data:
                print(f"В компании: {company} открыто {vacancies} вакансий.")
        elif len(data) == 5:
            # Обработка данных для вакансий
            company, title, salary_from, salary_to, url = data
            if salary_from == 0 and salary_to == 0:
                salary_str = "Зарплата: не указана"
            else:
                salary_str = f"Зарплата от: {salary_from} до {salary_to}"
            print(f"\nКомпания: {company}\nДолжность: {title}\n{salary_str}\nСсылка: {url}\n")
        elif len(data) == 4:
            company, title, salary_avg, url = data
            print(f"\nКомпания: {company}\nДолжность: {title}\nСредняя зарплата: {salary_avg}\nСсылка: {url}\n")
